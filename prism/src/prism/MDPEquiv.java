 ...
 *     from an EC state)</li>
 * <li>(tau, s, alpha) corresponds to an action alpha that originated from state s in some SC</li>
 * </ul>
 ...
 * <ul>
 * <li>State rewards for all EC states are set to zero</li>
 * <li>Transition rewards for all state-action pairs where all successors
 *     remain in the EC are set to zero as well (this is visible for the self-loops on
 *     representatives that would be deadlocks otherwise)</li>
 * </ul>
 *
 */
public class MDPEquiv extends NondetModelTransformation
{
	...
	public MDPEquiv(PrismComponent parent, NondetModel model, List<JDDNode> equivalentClasses)
	{
		super(model);
	...
	public JDDVars getTauVar()
	{
		JDDVars tauVar = new JDDVars();
		tauVar.addVar(extraActionVars.getVar(0).copy());
	...
	{
		JDDNode notTau = JDD.Not(tau());
		for (int i = 0; i < actFromStates.n(); i++) {
			notTau = JDD.And(notTau, JDD.Not(actFromStates.getVar(i).copy()));
		}
		return notTau;
	}
	...
	public JDDNode liftFromRepresentatives(JDDNode n)
	{
		...
		result = JDD.MaxAbstract(result, originalModel.getAllDDColVars());
		return result;
	}

	public static class MDPQuotient implements ModelTransformation<NondetModel, NondetModel>
	{

		private NondetModel originalModel;
		private NondetModel transformedModel;
		private MDPEquiv transform;
		private JDDNode transformedStatesOfInterest;

		private MDPQuotient(NondetModel originalModel, NondetModel transformedModel, MDPEquiv transform, JDDNode transformedStatesOfInterest)
		{
	...
	/**
	 * Compute the quotient MDP, collapsing each maximal end component to a single state.
	...
	{
		final MDPEquiv transform = new MDPEquiv(parent, model, equivalentClasses);
