...
public class PrismMTBDD
{
	...
	// export vector
	private static native int PM_ExportVector(long vector, String name, long vars, int nv, long odd, int exportType,
											  String filename, String rewardStructName, boolean noexportheaders);
	public static void ExportVector(JDDNode vector, String name, JDDVars vars, ODDNode odd, int exportType,
									String filename, int precision, String rewardStructName, boolean noexportheaders) throws FileNotFoundException
	{
		...
		int res = PM_ExportVector(vector.ptr(), name, vars.array(), vars.n(), odd.ptr(), exportType, filename,
				rewardStructName, noexportheaders);
		if (res == -1) {
